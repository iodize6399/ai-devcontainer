...
resolve_release_json() {
    ...
    requested_version="$(normalize_version "${OMP_VERSION:-latest}")"
...
map_platform() {
    if [ "$(uname -s)" != "Linux" ]; then
        echo "The oh-my-pi feature only supports Linux containers."
        exit 1
    fi
...
main() {
    ...
    if [ -z "$release_tag" ]; then
        echo "Unable to resolve an oh-my-pi release tag."
        exit 1
    fi
    ...
    local asset_name="omp-${PACKAGE_PLATFORM}"
    local download_url
    ...
    if [ -z "$download_url" ]; then
        ...
        printf '%s\n' "Available assets:"
        printf '%s' "$release_json" | jq -r '.assets[].name | select(startswith("omp-linux-"))'
        exit 1
    fi
    ...
    install -m 0755 "$archive_path" "${INSTALL_DIR}/${BINARY_NAME}"

    "${BINARY_NAME}" --version
    echo "Installed ${BINARY_NAME} ${release_version}."
}
